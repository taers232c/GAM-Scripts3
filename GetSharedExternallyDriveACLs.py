#!/usr/bin/env python3
"""
# Purpose: For a Google Drive User(s), show all drive file ACLs for files shared outside of a list of specified domains, this includes anyone
...
#    For Advanced GAM, add the following clause to the command for each domain in DOMAIN_LIST: pm not domain xyz.com em
#  $ Basic GAM: gam all users print filelist id title permissions owners > filelistperms.csv
#  $ Advanced GAM: gam config auto_batch_min 1 redirect csv ./filelistperms.csv multiprocess all users print filelist fields id,title,permissions,owners.emailaddress pm type anyone em pm not domain xyz.com em
